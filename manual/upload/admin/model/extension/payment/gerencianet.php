...
class ModelExtensionPaymentGerencianet extends Model {
     ...
     */
    public function createTablePix() {
		$this->db->query("
    ...
    /**
     * Cria tabela que relaciona o pedido com o pagamento via Open Finance
     */
    public function createTableOF() {
		$this->db->query("
            CREATE TABLE IF NOT EXISTS `{$this->getTableWithPrefix('gerencianet_of')}` (
                `order_id` integer PRIMARY KEY NOT NULL,
                `identificadorPagamento` varchar(255),
                `e2e_id` text,
                `status` text NOT NULL
            )"
        );
