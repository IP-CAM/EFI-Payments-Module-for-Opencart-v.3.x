...
class GatewayPix
{
     ...
     * @return array
     */
    private function getGerencianetInstance($dataConfig)
    {
        try {
            if (isset($dataConfig['pixCert'])) {
                $newInstance = new Gerencianet(
                    array(
                        // OpenCart retorna 1 para true e NULL para false
                        'client_id' => $dataConfig['sandbox'] == 1 ? $dataConfig['clientIdDev'] : $dataConfig['clientIdProd'],
                        'client_secret' => $dataConfig['sandbox'] == 1 ? $dataConfig['clientSecretDev'] : $dataConfig['clientSecretProd'],
                        'sandbox' => $dataConfig['sandbox'] == 1,
                        'certificate' => $dataConfig['pixCert'],
                        'debug' =>  $dataConfig['debug'] == 1,
                        'headers' => [
                            'x-skip-mtls-checking' => $dataConfig['mtls'] == 1 ? 'false' : 'true', //precisa ser string
    ...
    private function payChargeBillet($customer, $expire_at, $chargeId, $dadosBoleto)
    {
        ...
        try {
            $billet = $this->apiInstance->definePayMethod($params, $body);
    ...
    private function payChargeCard($customer, $chargeId, $addres)
    {
        ...
        try {
            $card = $this->apiInstance->definePayMethod($params, $body);
            return $card;
        } catch (\Throwable $th) {
            $validation = new GerencianetValidation();
            return $validation->getErrorMessage($th->getCode());
    ...
    public function startOpenFinancePayment($body)
    {
        try {
            $response = $this->apiInstance->ofStartPixPayment($params = [], $body);
            $response["code"] = 200;
            return $response;
        } catch (GerencianetException $e) {
            $errorDatails = ['mensagem' => $e->error, 'code' => $e->code];
            $this->registrarLog($errorDatails);
            return $errorDatails;
        } catch (Exception $e) {
            $errorDatails = ['mensagem' => $e->getMessage(), 'code' => $e->getCode()];
            $this->registrarLog($errorDatails);
            return $errorDatails;
        }
    }
    public function getParticipantsOF()
    {
        return $this->apiInstance->ofListParticipants()['participantes'];
    }
    ...
    private function registrarLog($logEfi)
    {
        $logString = json_encode( $logEfi);
        // Caminho para o arquivo de log
        $arquivoDeLog = DIR_LOGS . 'efi_bank.log';

        // Gravando a mensagem no arquivo de log
        file_put_contents($arquivoDeLog, 'Open Finance Erro: '. date('Y-m-d H:i:s') . ' - ' . $logString . PHP_EOL, FILE_APPEND);
    }
}
