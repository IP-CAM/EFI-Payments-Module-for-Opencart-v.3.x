try {
    ...
    } else if ($("#gerencianetPix").length > 0) {
        $("#gerencianetPix").attr('checked', true);
    }else{
        $("#gerencianetOF").attr('checked', true);
    }
    ...
    $("#gerencianetOF_click").click(() => {
        $('.form-payment-gerencianet :submit').removeAttr('disabled');
    
        $("#gerencianetOF").prop('checked', true);
        $("#creditSelected").fadeOut(0);
        $("#billetSelected").fadeOut(0);
        $("#pixSelected").fadeOut(0);
        $("#ofSelected").show(700);
    ...
    setTimeout(function () {
        ...
        maskDocumentInputsGerencianet(['#documentClientBillet', '#documentClientCredit', '#documentClientPix','#documentClientCpfOF','#documentClientCnpjOF']);
    ...
    $('.form-payment-gerencianet').submit((event) => {
        const isActivated = (name) => !!document.getElementById(name);
    
        if (isActivated('gerencianetBoleto') && document.getElementById('gerencianetBoleto').checked) {
            validationForm('billet', event);
        } else if (isActivated('gerencianetPix') && document.getElementById('gerencianetPix').checked) {
            validationForm('pix', event);
        } else if (isActivated('gerencianetCartao') && document.getElementById('gerencianetCartao').checked) {
            validationForm('credit', event);
        } else if (isActivated('gerencianetOF') && document.getElementById('gerencianetOF').checked) {
            validationForm('openFinance', event);
        }
    });
    
    
    function verifyMinimumValue() {
        if ($('#totalPedido').val().replace(',', '.') < 3) {
            $('#billetSelected').html('<h4 class="text-justify   payment-description-title"><strong>Valor mínimo de compra para boleto: <span class="text-danger">R$3,00</span></strong></h4>');
    
            $('#creditSelected').html('<h4 class="text-justify   payment-description-title">	<strong>Valor mínimo de compra para cartão: <span class="text-danger">R$3,00</span></strong></h4>');
