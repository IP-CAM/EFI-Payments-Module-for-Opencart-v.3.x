...
function validationOpenFinance(event) {
    ...
    $('.form-payment-gerencianet input').css('border-color', 'rgb(204, 204, 204)');
    $('.form-payment-gerencianet select').css('border-color', 'rgb(204, 204, 204)');
    let documentClientOFCPF = $('#documentClientCpfOF').val().replaceAll('.', '').replace('-', '').replace('/', '');
    let documentClientOFCNPJ = $('#documentClientCnpjOF').val().replaceAll('.', '').replace('-', '').replace('/', '');
    let bancoFinalizacaoOpenFinance =  $('#bancoOF').select2('data')[0].id;
    
    console.log(bancoFinalizacaoOpenFinance)
   
    if (bancoFinalizacaoOpenFinance == '') {
        event.preventDefault();
        erros++;
        $('#alert-bank-open-finance').append('<p class="text-danger"> <strong>Seleicone um Banco.</strong></p>');
        
    }

    if (!verifyCPF(documentClientOFCPF)) {
        event.preventDefault();
        erros++;
        $('#documentClientCpfOF').after('<p class="text-danger"> <strong>CPF inválido.</strong></p>');
        $('#documentClientCpfOF').css('border-color', '#a94442');

    } else if(documentClientOFCPF.lenght == 0) {
        event.preventDefault();
        erros++;
        $('#documentClientCpfOF').after('<p class="text-danger"><strong> Campo obrigatório.</strong></p>');
        $('#documentClientCpfOF').css('border-color', '#a94442');
    }
    if (documentClientOFCNPJ.length > 0) {
        if (!verifyCNPJ(documentClientOFCNPJ)) {
            event.preventDefault();
            erros++;
            $('#documentClientCnpjOF').after('<p class="text-danger"><strong> CNPJ inválido.</strong> </p>');
            $('#documentClientCnpjOF').css('border-color', '#a94442');
        }
    }
